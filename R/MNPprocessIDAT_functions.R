...
MNPnormalize.illumina.control <- function (rgSet, ref=10000) {
  ...
  if (minfi:::.is450k(rgSet) || minfi:::.isEPIC(rgSet)) {
    AT.controls <- minfi::getControlAddress(rgSet, controlType = c("NORM_A", "NORM_T"))
    CG.controls <- minfi::getControlAddress(rgSet, controlType = c("NORM_C", "NORM_G"))
  }
  if (minfi:::.is27k(rgSet)) {
    AT.controls <- minfi::getControlAddress(rgSet, controlType = "Normalization-Red")
    CG.controls <- minfi::getControlAddress(rgSet, controlType = "Normalization-Green")
  }
